...
class Radio {
    ...
    public function getRadios($page = 1, $limit = 20, $filters = []) {
        ...
        if (!empty($filters['search'])) {
            $where_conditions[] = "(MATCH(radio_name, brief_description, detailed_description) AGAINST(:search IN NATURAL LANGUAGE MODE) OR radio_name LIKE :search_like)";
    ...
    public function createRadio($data) {
        ...
        $stmt->bindParam(':genres', json_encode($data['genres']));
        ...
        throw new Exception('Erro ao cadastrar rádio');
    }
